...
async function main() {
  try {
    ...
    const subscriptionPayments = razorpayPayments.filter(p => p.amount === 299);

    console.log(`\n📊 Payment Summary:`);
    console.log(`   Total Razorpay payments: ${razorpayPayments.length}`);
    console.log(`   Ticket payments: ${ticketPayments.length}`);
    console.log(`   Subscription payments: ${subscriptionPayments.length}`);

    // Check all ticket payments
    let foundPayments = 0;
    let missingPayments = [];
    ...
    for (const paymentData of ticketPayments) {
      ...
      if (existingPayment) {
        ...
        missingPayments.push(paymentData);
      }
    }

    console.log(`\n✅ Ticket Payments Status:`);
    console.log(`   Found in database: ${foundPayments}/${ticketPayments.length}`);
    console.log(`   Missing: ${missingPayments.length}`);

    if (missingPayments.length > 0) {
      console.log(`\n   ⚠️  Missing payments:`);
      missingPayments.forEach(p => {
        console.log(`      - ${p.paymentId} (${p.email}, ₹${p.amount})`);
      });
    }

    // Check specific users with multiple payments
    console.log(`\n\n🔍 Users with Multiple Payments:`);
    
    // suyash.verma017@gmail.com
    ...
    if (suyashUser) {
      ...
      const suyashPayments = ticketPayments.filter(p => p.email === 'suyash.verma017@gmail.com');
      console.log(`\n   ✅ suyash.verma017@gmail.com:`);
      console.log(`      Expected: ${suyashPayments.length} payments`);
      console.log(`      Tickets: ${suyashTickets.length}`);
      console.log(`      Payment IDs: ${suyashTickets.map(t => t.payments.map(p => p.gatewayTxnId).join(', ')).join(', ')}`);
    }

    // matlanichehek@gmail.com
    ...
    if (chehekUser) {
      ...
      const chehekPayments = ticketPayments.filter(p => p.email === 'matlanichehek@gmail.com');
      console.log(`\n   ✅ matlanichehek@gmail.com:`);
      console.log(`      Expected: ${chehekPayments.length} payments`);
      console.log(`      Tickets: ${chehekTickets.length}`);
      console.log(`      Payment IDs: ${chehekTickets.map(t => t.payments.map(p => p.gatewayTxnId).join(', ')).join(', ')}`);
    }

    // saurav.dayal.39@gmail.com
    ...
    if (sauravUser) {
      ...
      console.log(`\n   ✅ saurav.dayal.39@gmail.com:`);
      console.log(`      Tickets: ${sauravTickets.length} (should be 1)`);
      if (sauravTickets.length > 0) {
        console.log(`      Payment ID: ${sauravTickets[0].payments.map(p => p.gatewayTxnId).join(', ') || 'None'}`);
      }
    }

    // rishab1065@gmail.com
    ...
    if (rishabUser) {
      ...
      console.log(`\n   ✅ rishab1065@gmail.com:`);
      console.log(`      Tickets: ${rishabTickets.length} (should be 1)`);
      if (rishabTickets.length > 0) {
        console.log(`      Payment ID: ${rishabTickets[0].payments.map(p => p.gatewayTxnId).join(', ') || 'None'}`);
      }
    }
    ...
    const ticketsWithPayments = allTickets.filter(t => t.payments.length > 0);
    const ticketsWithoutPayments = allTickets.filter(t => t.payments.length === 0);

    console.log(`\n\n📊 Final Database State:`);
    console.log(`   Total tickets: ${allTickets.length}`);
    console.log(`   Tickets with payments: ${ticketsWithPayments.length}`);
    console.log(`   Tickets without payments: ${ticketsWithoutPayments.length}`);
    console.log(`   Total ticket quantity: ${allTickets.reduce((sum, t) => sum + t.quantity, 0)}`);
    console.log(`   Event soldTickets: ${event.soldTickets}`);
