...
// List of all payments from Razorpay dashboard
const payments = [
...
async function main() {
  try {
    console.log(`🔍 Checking ${payments.length} payments against database...\n`);
    ...
    for (const paymentData of payments) {
      try {
        ...
        if (existingPayment) {
          console.log(`⏭️  Skipped: ${paymentData.paymentId} - ${paymentData.email} (already exists)`);
          skipped++;
          continue;
        }
        ...
        if (!user) {
          user = await prisma.user.create({
            data: {
              ...
              phone: paymentData.phone.replace(/\D/g, '').slice(-10), // Clean phone number
              role: 'USER',
            }
          });
          console.log(`👤 Created user: ${paymentData.email}`);
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
          ...
          const payment = await tx.payment.create({
            data: {
              ...
              paymentMethod: 'UPI',
              gateway: 'RAZORPAY',
              gatewayOrderId: `order_${paymentData.paymentId.slice(4)}`, // Generate order ID
              gatewayTxnId: paymentData.paymentId,
              gatewayResponse: {
