...
async function main() {
  try {
    console.log('🔍 Checking which Razorpay payments are already in database...\n');
    console.log(`Total Razorpay payments to check: ${razorpayPayments.length}\n`);
    ...
    for (const razorpayPayment of razorpayPayments) {
      const dbPayment = await prisma.payment.findFirst({
        where: {
          gatewayTxnId: razorpayPayment.paymentId
        },
        include: {
          ticket: {
            include: {
      ...
      if (dbPayment) {
        foundPayments.push({
          ...
          db: {
            ...
            ticketId: dbPayment.ticket?.id,
            amount: dbPayment.amount,
            quantity: dbPayment.ticket?.quantity,
            userEmail: dbPayment.ticket?.user?.email,
            userName: dbPayment.ticket?.user?.name,
            eventTitle: dbPayment.ticket?.event?.title,
            createdAt: dbPayment.createdAt
          }
        });
      } else {
        missingPayments.push(razorpayPayment);
      }
    }

    console.log('✅ PAYMENTS FOUND IN DATABASE:\n');
    console.log(`Total found: ${foundPayments.length}\n`);
    foundPayments.forEach((item, index) => {
      console.log(`${index + 1}. ${item.razorpay.paymentId}`);
      console.log(`   Email: ${item.razorpay.email}`);
      console.log(`   Amount: ₹${item.razorpay.amount} (DB: ₹${item.db.amount})`);
      console.log(`   Quantity: ${item.db.quantity || 'N/A'}`);
      console.log(`   Event: ${item.db.eventTitle || 'N/A'}`);
      console.log(`   Created: ${new Date(item.db.createdAt).toLocaleString()}`);
      console.log('');
    });

    console.log('\n\n❌ PAYMENTS MISSING FROM DATABASE:\n');
    ...
    if (missingPayments.length > 0) {
      ...
      console.log('✅ All payments are present in database!');
    }

    console.log('\n\n📊 SUMMARY:');
    console.log(`   Total Razorpay payments: ${razorpayPayments.length}`);
    console.log(`   Found in database: ${foundPayments.length}`);
