...
async function main() {
  try {
    console.log('🔍 Comprehensive check for all payments and tickets...\n');
    ...
    console.log('📊 Checking all users with payments...\n');
      ...
      for (const ticket of tickets) {
        for (const payment of ticket.payments) {
          if (payment.gatewayTxnId) {
            paymentIds.add(payment.gatewayTxnId);
          }
        }
      }

      const expectedPaymentIds = payments.map(p => p.paymentId);
      const missingPayments = expectedPaymentIds.filter(id => !paymentIds.has(id));
      const extraTickets = tickets.length - payments.length;

      if (missingPayments.length > 0 || extraTickets > 0) {
        issuesFound++;
        console.log(`\n📧 ${email}:`);
        console.log(`   Expected: ${payments.length} payment(s)`);
        console.log(`   Found: ${tickets.length} ticket(s)`);
        console.log(`   Payment IDs in DB: ${Array.from(paymentIds).join(', ') || 'None'}`);
        
        if (missingPayments.length > 0) {
          console.log(`   ⚠️  Missing payments: ${missingPayments.join(', ')}`);
        }
        
        if (extraTickets > 0) {
          console.log(`   ⚠️  Extra tickets: ${extraTickets} (should be ${payments.length})`);
        }
      }
    }
    ...
    console.log(`   Expected payments: ${razorpayPayments.length}`);
